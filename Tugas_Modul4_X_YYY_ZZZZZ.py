...
def scatter(model, model_name, data, new_point, features, color_scale, title):
    ...
    fig.add_trace(
        go.Scatter3d(
            x=new_point['Avg_Credit_Limit'], y=new_point['Total_Credit_Cards'],
            z=new_point['Total_visits_online'],
            mode='markers',
            marker=dict(size=10, color='red'), name='New Point'
        )
    )
...
st.set_page_config(
    page_title="11775 - Unsupervised Learning", 
    page_icon="",
    layout="wide",
...
if uploaded_file is not None:
    ...
    st.markdown("<h1 style='text-align: center;'>Unsupervised Learning - Erin</h1>", unsafe_allow_html=True)
    st.dataframe(input_data)

    model_directory = r'C:\Users\ASUS\Downloads\Unsupervised Learning (Praktek)\Unsupervised Learning (Praktek)\Tugas4_B_11775'
    model_path = {
    ...
    su_CC = st.sidebar.number_input("Total Credit Cards", 0, 10)
